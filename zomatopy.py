...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", limit=5):
        ...
        req = base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(
            latitude) + "&lon=" + str(
            longitude) + "&cuisines=" + str(cuisines) + "&sort=rating&order=desc"
        print("restaurant_search::: " + req)
        r = (requests.get(req, headers=headers).content).decode("utf-8")
        return r  # a = ast.literal_eval(r)

    def restaurant_search_iteratively(self, query="", latitude="", longitude="", cuisines="", start=0, sort="rating",
                                      order="desc"):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        cuisines = "%2C".join(cuisines.split(","))
        if str(start).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}

        req = base_url + "search?q=" + str(query) + "&start=" + str(start) + "&lat=" + str(
            latitude) + "&lon=" + str(
            longitude) + "&cuisines=" + str(cuisines) + "&sort=" + sort + "&order=" + order
        print("restaurant_search::: " + req)
        r = (requests.get(req, headers=headers).content).decode("utf-8")
        return r  # a = ast.literal_eval(r)
    ...
    def get_location(self, query="", limit=5):
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        print("get_location::: " + base_url + "locations?query=" + str(query) + "&count=" + str(limit))
    ...
    def restaurant_search_by_keyword(self, query="", cuisines="", limit=5):
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        print("restaurant_search_by_keyword::: " + base_url + "search?q=" + str(query) + "&count=" + str(
            limit) + "&cuisines=" + str(cuisines))
